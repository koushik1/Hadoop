...
public class TFICF {
    ...
    public static class WCMapper extends Mapper<Object, Text, Text, IntWritable> {
        ...
        public void map(Object key, Text value, Context context
            ...
            String fileName = ((FileSplit) context.getInputSplit()).getPath().getName(); //Get the document name

	    //Read the input word by word
            while (itr.hasMoreTokens()) {
                String current_word = itr.nextToken();
                current_word = current_word.toLowerCase().trim();
                current_word = current_word.replaceAll("[^a-zA-Z0-9\\u0080-\\u9fff-\\[\\]`/]", ""); //Replace unwanter characters

		//If string is not empty and begins with a letter
	...
	public void reduce(Text key, Iterable<IntWritable> values,Context context
	    ...
	    //Sum to find th number of occurences
	    int sum = 0;
	...
	/************ YOUR CODE HERE ************/
	    ...
	    String[] keys = value.toString().split("\\t");//Split by tab space
	    String word_doc = keys[0];
	    String word_count = keys[1];
	    String[] doc_key = word_doc.split("@");//Split the original key by @
	    String current_word = doc_key[0];
	    String document = doc_key[1];
	    //Generate the new key and value pair
	...
	/************ YOUR CODE HERE ************/
	    ...
	    String[] keys = value.toString().split("\\t");//Split by tab
	    String word_doc = keys[0];
	    String word_count_per_docSize = keys[1];
	    String[] doc_key = word_doc.split("@");//Split the original key by alpha
	    String current_word = doc_key[0];
	    String document = doc_key[1];
	    //generate the new key value pairs
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
	    ...
	    /************ YOUR CODE HERE ************/
	    // Add the values for the key to the list and also find the number of documents it appears in
